...
class Project(models.Model):
    ...
    project_type = models.CharField(
        max_length=3, choices=PROJECT_TYPE_CHOICES, verbose_name="Type de projet"
    )
    ...
    def add_contributor(self, user, contribution):
        _, created = ProjectContributor.objects.get_or_create(
            project=self, contributor=user, defaults={"contribution": contribution}
        )
        if not created:
...
class ProjectContributor(models.Model):
    ...
    def __str__(self):
        return (
            f"{self.contributor.username} - {self.project.name} ({self.contribution})"
        )
...
class Issue(models.Model):
    ...
    project = models.ForeignKey(
        Project, on_delete=models.CASCADE, related_name="issues", verbose_name="Projet"
    )
...
class Comment(models.Model):
    ...
    issue = models.ForeignKey(
        Issue, on_delete=models.CASCADE, related_name="comments", verbose_name="Issue"
    )
    author = models.ForeignKey(
        settings.AUTH_USER_MODEL,
        on_delete=models.CASCADE,
